...
namespace AngModel {
	class Vect {
		...
		public Vect getShiftSide(double shift_side) {
			double ln = len;
			if(ln == 0)
				return new Vect(a, b);
			double dy = b.Y - a.Y;
			...
			double dx = b.X - a.X;
			double cosa = dx / ln;
			double shiftx = shift_side * sina;
			double shifty = -shift_side * cosa;
			Point newa = new Point(a.X + shiftx, a.Y + shifty);
			Point newb = new Point(b.X + shiftx, b.Y + shifty);
